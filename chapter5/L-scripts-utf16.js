...
function filter(array, test) {
  ...
  for (const element of array) if (test(element)) passed.push(element);
...
const ttbScripts = SCRIPTS.filter((SCRIPT) => SCRIPT.direction == "ttb");

function map(array, transform) {
  const transformed = [];

  for (const element of array) transformed.push(transform(element));

  return transformed;
}

console.log(map(ttbScripts, (SCRIPT) => SCRIPT.name));
...
console.log(
  SCRIPTS.reduce((a, b) => (countCharacters(a) > countCharacters(b) ? a : b))
);
...
function countBy(array, groupName) {
  ...
  for (const element of array) {
    const name = groupName(element);
    const known = counted.findIndex((count) => count.name == name);
    if (known == -1) {
      counted.push({ name, count: 1 });
    } else {
      counted[known].count++;
    }
  }
...
console.log(countBy([2, 4, 6, 1, 3], (n) => (n % 2 ? "odd" : "even")));
...
function textScripts(text) {
  ...
  const total = scripts.reduce((sum, { count }) => sum + count, 0);
